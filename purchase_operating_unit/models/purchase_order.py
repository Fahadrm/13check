...
class PurchaseOrder(models.Model):
    ...
    @api.model
    def get_operating_unit_user(self):
        if self.env.uid == 1 and self.env.uid != self.env.context["uid"]:
            return self.env.context["uid"]
        else:
            return self.env.uid
    ...
    operating_unit_id = fields.Many2one(
        ...
        default=lambda self: (
            self.env["res.users"].operating_unit_default_get(self.get_operating_unit_user())
        ),
    )
    ...
    def _check_warehouse_operating_unit(self):
        for record in self:
            ...
            if record.env.uid != 1 and record.env.uid == record.env.context["uid"]:
